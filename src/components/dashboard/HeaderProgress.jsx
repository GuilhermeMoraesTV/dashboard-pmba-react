...
function HeaderProgress({ registrosEstudo = [], goalsHistory = [], activeCicloId, onTriggerGuide, onShareGoal }) {
    ...
    const stats = useMemo(() => {
        const safeGoals = Array.isArray(goalsHistory) ? goalsHistory : [];
        const activeGoal = safeGoals.length > 0
            ? [...safeGoals].sort((a, b) => new Date(b.startDate) - new Date(a.startDate))[0]
            : { questions: 0, hours: 0 };
        ...
        const safeRegistros = Array.isArray(registrosEstudo) ? registrosEstudo : [];

        safeRegistros.forEach(reg => {
    ...
    const statusConfig = {
        ...
        working: {
            ...
            barFill: 'bg-blue-50 shadow-[0_0_10px_rgba(59,130,246,0.5)]',
            label: 'Em Foco',
    ...
    return (
        ...
        >
            ...
            <motion.div
                ...
                whileTap={{ scale: 0.98 }}
                onClick={handleToggle} // ADICIONADO: Clique para abrir/fechar
                className={`
                    flex items-center gap-3 px-2 py-1.5 sm:px-3 sm:py-2
                    border rounded-2xl cursor-pointer
                    backdrop-blur-xl transition-all duration-300
                    /* Mobile: Fundo transparente para parecer solto */
                    ${currentConfig.containerClass}
                    sm:${currentConfig.containerClass}
                    bg-transparent border-transparent shadow-none sm:bg-white sm:border-zinc-200 sm:shadow-sm sm:dark:bg-zinc-900 sm:dark:border-zinc-800
                `}
            >
                {/* 1. Ícone com Fundo Colorido */}
                <div className={`
                    flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl shrink-0
                    ${currentConfig.iconBg} transition-colors duration-300 relative
                `}>
                    <StatusIcon size={18} className="sm:w-5 sm:h-5" strokeWidth={2.5} />

                    {/* MOBILE ONLY: Porcentagem como "Potenciação" (Superscript) */}
                    <div className={`
                        sm:hidden absolute -top-1 -right-3
                        text-[10px] font-black leading-none
                        ${currentConfig.mobileColor || 'text-zinc-600 dark:text-zinc-300'}
                    `}>
                        {stats.overallProgress.toFixed(0)}%
                    </div>
                </div>

                {/* 2. Área Central (Texto e Barra) - HIDDEN ON MOBILE */}
                <div className="hidden sm:flex flex-col justify-center flex-1 min-w-[90px] sm:min-w-[140px]">
                ...
                {/* 3. Indicador de Expansão (Desktop Only) */}
            ...
            <AnimatePresence>
                {isExpanded && (
                    <motion.div
                        ...
                        className="absolute top-full right-0 mt-2 sm:mt-4 w-[280px] sm:w-[400px]
                        ...
                        {showCelebration ? (
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    {/* Card Horas */}
                                    <div className="flex flex-col items-center p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-700/50 group">
                                        <span className="text-xs font-bold text-zinc-400 uppercase mb-3">Tempo</span>
                                    ...
                                    {/* Card Questões */}
                                    <div className="flex flex-col items-center p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-100 dark:border-zinc-700/50 group">
                                        <span className="text-xs font-bold text-zinc-400 uppercase mb-3">Questões</span>
                                        <DetailedProgressRing
                                            percent={stats.percQ}
