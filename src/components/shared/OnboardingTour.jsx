...
const TOURS = {
  main: [
    ...
    },
    {
      ...
      content: 'Depois de definir suas metas, utilize esta barra no topo para acompanhar se o objetivo do dia (Tempo e Questões) está sendo cumprido. Você também poderá compartilhar seu relatório diário aqui.',
      alignment: 'bottom',
      hasMascot: true,
      mascotSide: 'right'
    },
...
// Função auxiliar para propriedades dinâmicas
const getDynamicStepProps = (step, isMobile) => {
    ...
    if (step.id === 'nav-metas') {
        if (isMobile) {
            return { alignment: 'top-left', mascotSide: 'left' };
        } else {
            return { alignment: 'top-right', mascotSide: 'left' };
        }
    }
    return {
...
const OnboardingTour = ({ tourType = 'main', isActive, onClose, onFinish, activeTab, setActiveTab }) => {
  ...
  let currentMascotSide = dynamicProps.mascotSide;
  ...
  // --- CONTROLE DE SINALIZAÇÃO DE TOUR ATIVO (NOVO) ---
  // Dispara eventos para bloquear/liberar notificações de XP
  useEffect(() => {
    if (isActive) {
        // Sinaliza que o tour começou -> XP deve esperar
        window.dispatchEvent(new CustomEvent('tour-active', { detail: true }));
    } else {
        // Sinaliza que o tour acabou -> XP liberado
        window.dispatchEvent(new CustomEvent('tour-active', { detail: false }));
    }

    // Cleanup: Garante que ao desmontar, libera o XP (caso algo quebre)
    return () => {
        window.dispatchEvent(new CustomEvent('tour-active', { detail: false }));
    };
  }, [isActive]);
  ...
  if (step.tab && activeTab !== step.tab && !isIntroStep) return null;

  const isCycleVisualButTargetMissing = (tourType === 'cycle_visual' && !targetRect && !isNavigating);
  if (isCycleVisualButTargetMissing) return null;

  // --- RENDERIZAÇÃO: INTRODUÇÃO ---
  if (isIntroStep) {
    return (
      <AnimatePresence>
        <div className="fixed inset-0 z-[99999] flex items-center justify-center px-4 font-sans">
          ...
          >
            <div className="h-24 bg-gradient-to-r from-red-600 to-orange-500 relative overflow-hidden">
               ...
               <div className="absolute -bottom-10 -right-10 text-white/10"><Sparkles size={120} /></div>
            </div>
            <div className="px-6 pb-6 pt-0 flex flex-col items-center text-center -mt-12 relative z-10 md:px-8 md:pb-8">
               <div className="w-28 h-28 md:w-32 md:h-32 bg-white dark:bg-zinc-800 rounded-full p-1 shadow-xl mb-6 flex items-center justify-center border-4 border-white dark:border-zinc-700">
                  <img src={mascotImage} alt="Mascote" className="w-20 h-auto md:w-24 drop-shadow-md" />
               </div>
               <h2 className="text-xl md:text-3xl font-black text-zinc-800 dark:text-white mb-2 leading-tight">{step.title}</h2>
               <p className="text-sm md:text-base text-zinc-600 dark:text-zinc-300 leading-relaxed mb-6 md:mb-8">{step.content}</p>
               <button onClick={handleNext} className="w-full py-3 md:py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-base md:text-lg shadow-lg shadow-red-500/20 transition-all transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2 group">
                 Iniciar o Guia <ChevronRight size={20} className="group-hover:translate-x-1 transition-transform" />
               </button>
               <button onClick={onClose} className="mt-3 text-xs md:text-sm text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 font-medium transition-colors">Pular apresentação</button>
            </div>
          </motion.div>
        </div>
      </AnimatePresence>
    ...
    <div className="fixed inset-0 z-[99999] overflow-hidden font-sans pointer-events-auto">
        ...
        <AnimatePresence mode="wait">
            {step?.hasMascot && !isIntroStep && (
                ...
                >
                    <img src={mascotImage} alt="Instrutor" className={`w-full h-auto drop-shadow-2xl filter brightness-110 ${isMascotRight ? 'scale-x-[-1]' : ''}`} />
                </motion.div>
            )}
        </AnimatePresence>
        ...
        <div className={`absolute inset-0 pointer-events-none flex p-2 md:p-8 z-[100000] ${getBoxPositionClass()}`}>
            <motion.div
                ...
                className="bg-zinc-900 text-white p-0 rounded-2xl shadow-2xl max-w-[180px] sm:max-w-sm md:max-w-md w-full pointer-events-auto relative border border-zinc-700 overflow-hidden flex flex-col mb-20 md:mb-0"
            >
                <div className="bg-gradient-to-r from-red-700 to-red-600 p-1.5 px-3 flex justify-between items-center md:p-3 md:px-5">
                    <span className="font-black uppercase tracking-widest text-[0.6rem] md:text-xs flex items-center gap-1"><MapPin size={10} className="md:w-auto" /> Guia {currentStepIndex + 1}/{steps.length}</span>
                    <button onClick={onClose} className="text-white/50 hover:text-white transition-colors"><X size={14} className="md:w-auto" /></button>
                </div>
                <div className="p-3 md:p-6">
                    <h2 className="text-xs md:text-xl font-black text-white mb-1 uppercase leading-tight">{step.title}</h2>
                    <p className="text-[0.65rem] md:text-sm text-zinc-300 mb-3 md:mb-6 leading-normal border-l-2 border-red-600 pl-2 bg-white/5 py-0.5 rounded-r-lg">{step.content}</p>
                    <div className="flex justify-end gap-2">
                        <button onClick={onClose} className="px-2 py-0.5 text-[0.6rem] md:text-xs font-bold text-zinc-500 hover:text-zinc-300 transition-colors uppercase">Pular</button>
                        <button onClick={handleNext} className="flex items-center gap-1 bg-white text-red-700 hover:bg-zinc-200 px-2 py-0.5 rounded-lg font-black uppercase tracking-tight shadow-lg transition-all transform hover:scale-105 active:scale-95 text-[0.65rem] md:text-xs">
                            {step.isLast ? 'Concluir' : 'Próximo'} {step.isLast ? <CheckCircle2 size={12} /> : <ChevronRight size={12} />}
                        </button>
                    </div>
                </div>
            </motion.div>
