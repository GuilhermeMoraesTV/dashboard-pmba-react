...
import { CheckCircle2, Clock3, Target, Trophy, X } from 'lucide-react';
      ...
      {open && (
        ...
        >
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-label="Meta do dia batida"
            className="relative w-[calc(100vw-1rem)] max-w-[340px] overflow-hidden rounded-[24px] border border-emerald-200/90 bg-white/95 shadow-2xl shadow-emerald-950/25 backdrop-blur-sm dark:border-emerald-900/40 dark:bg-zinc-950/95 sm:aspect-square sm:w-full sm:max-w-[520px] sm:rounded-[30px]"
          ...
          >
            ...
            <div className="pointer-events-none absolute left-8 right-8 top-[132px] h-px bg-gradient-to-r from-transparent via-emerald-300 to-transparent dark:via-emerald-800" />
            <motion.div
            ...
            <motion.img
              src={systemLogo}
              alt="ModoQAP"
              className="absolute left-4 top-4 z-20 h-12 w-12 object-contain drop-shadow-xl sm:left-6 sm:top-6 sm:h-[86px] sm:w-[86px]"
            ...
            <div className="relative flex min-h-full flex-col px-3.5 pb-3.5 pt-4 sm:h-full sm:px-7 sm:pb-7 sm:pt-7">
              <header className="mx-auto flex min-h-[154px] max-w-[280px] flex-col items-center justify-center px-9 text-center sm:min-h-[224px] sm:max-w-[360px] sm:px-16">
                <motion.span
                  className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/90 px-3 py-1 text-[8px] font-black uppercase tracking-[0.26em] text-emerald-700 shadow-sm dark:border-emerald-900/50 dark:bg-zinc-900/80 dark:text-emerald-300"
                ...
                >
                  ...
                  />
                  <motion.div
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-lg shadow-emerald-500/30 sm:h-14 sm:w-14"
                  ...
                  >
                    ...
                    >
                      <Trophy className="h-5 w-5 sm:h-[30px] sm:w-[30px]" strokeWidth={2.5} />
                    </motion.div>
                  </motion.div>
                </motion.div>
                <motion.h2
                  className="mt-3 inline-flex max-w-full flex-wrap items-center justify-center gap-2 text-center text-base font-black uppercase leading-none tracking-normal text-zinc-950 dark:text-white sm:mt-4 sm:gap-2.5 sm:text-[1.55rem]"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.35, delay: 0.46, ease: 'easeOut' }}
                >
                  <span>Meta do dia batida</span>
                  <motion.span
                    className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-amber-200 bg-amber-50 text-amber-600 shadow-sm dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-300"
                    animate={{ rotate: [0, -8, 8, -4, 0], scale: [1, 1.1, 1] }}
                    transition={{ duration: 2.4, repeat: Infinity, repeatDelay: 0.5, ease: 'easeInOut' }}
                  >
                    <Trophy size={19} strokeWidth={2.4} />
                  </motion.span>
                </motion.h2>
                <motion.p
                  className="mt-2 text-[9px] font-black uppercase tracking-[0.22em] text-emerald-600 dark:text-emerald-400"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
              ...
              <section className="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-[0.92fr_1.08fr] sm:gap-3">
                <motion.div
                  className="relative flex min-h-0 flex-col overflow-hidden rounded-[20px] border border-zinc-200 bg-white/80 p-2.5 shadow-sm dark:border-white/10 dark:bg-zinc-900/80 sm:rounded-[24px] sm:p-3.5"
                ...
                >
                  ...
                  <p className="mt-2 text-[8px] font-black uppercase tracking-[0.24em] text-emerald-600 dark:text-emerald-400">Edital</p>
                  <div className="flex flex-1 items-center gap-3 text-left sm:flex-col sm:justify-center sm:text-center">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[16px] border border-zinc-100 bg-white p-1.5 shadow-sm dark:border-white/10 dark:bg-zinc-950 sm:h-20 sm:w-20 sm:rounded-[22px] sm:p-2.5">
                      {editalLogo ? (
                    ...
                    <h3 className="min-w-0 text-xs font-black uppercase leading-tight text-zinc-950 dark:text-white sm:text-sm">
                      {displayEditalName}
                    </h3>
                  </div>
                </motion.div>
                ...
                <div className="grid min-h-0 grid-cols-2 gap-2">
                  {stats.map(({ label, value, icon: Icon, tone }, index) => (
                    <motion.div
                      key={label}
                      className={`relative overflow-hidden rounded-[16px] border px-2 py-2 shadow-sm sm:rounded-[20px] sm:px-3 sm:py-3 ${
                        tone === 'emerald'
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="text-[7px] font-black uppercase leading-tight tracking-widest text-zinc-500 dark:text-zinc-400 sm:text-[8px]">{label}</p>
                          <p className="mt-1.5 text-lg font-black leading-none text-zinc-950 dark:text-white sm:mt-2 sm:text-xl">{value}</p>
                        </div>
                        <div
                          className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-xl sm:h-7 sm:w-7 ${
                            tone === 'emerald'
              ...
              >
                ...
                <span className="text-[8px] font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400">100%</span>
              </motion.div>
            </div>
          </motion.div>
        </motion.div>
