...
const LOCAL_KEY = '@ModoQAP:TimerSettings';

// ✅ FIX: hook agora aceita userUid para sincronizar com Firestore entre dispositivos.
// Mantém localStorage como cache local para leitura instantânea no primeiro render.
export const useTimerSettings = (userUid = null) => {
  const [settings, setSettings] = useState(() => {
    // Inicializa do localStorage para resposta imediata (sem flash de valores padrão)
    try {
  ...
  // ✅ Listener do Firestore: atualiza settings em tempo real em todos os dispositivos
  useEffect(() => {
    if (!userUid) return;

    const settingsDocRef = doc(db, 'users', userUid, 'settings', 'timerSettings');

    const unsub = onSnapshot(settingsDocRef, (snap) => {
      if (!snap.exists()) return;

      const remoteData = snap.data();
      // Ignora atualizações que vieram deste próprio dispositivo (já aplicadas via localStorage)
      const merged = { ...DEFAULT_SETTINGS, ...remoteData };

      // Atualiza estado e cache local
      setSettings(merged);
      try { localStorage.setItem(LOCAL_KEY, JSON.stringify(merged)); } catch {}
    }, (err) => {
      console.warn('[TimerSettings] Erro no snapshot:', err);
    });

    return () => unsub();
  }, [userUid]);

  // ✅ updateSettings salva em ambos: localStorage (instantâneo) + Firestore (cross-device)
  const updateSettings = useCallback(async (newSettings) => {
    ...
    // 2. Persiste no Firestore para outros dispositivos (sem bloquear a UI)
    if (userUid) {
      try {
        const settingsDocRef = doc(db, 'users', userUid, 'settings', 'timerSettings');
        await setDoc(settingsDocRef, updated, { merge: true });
      } catch (err) {
        console.warn('[TimerSettings] Falha ao salvar no Firestore:', err);
      }
    }
  }, [userUid]);
...
// ─────────────────────────────────────────────
// Sub-componente: ajustador de tempo
// ─────────────────────────────────────────────
