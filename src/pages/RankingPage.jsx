...
import { ArrowDown, ArrowUp, Award, BarChart3, Clock3, Crown, Flame, ListChecks, Medal, Shield, Sparkles } from 'lucide-react';
...
import { isLiveRankingMember } from '../utils/liveStudyTimer';
...
const metricValue = (member, metric) => metric === 'minutes'
  ? formatStudyMinutes(member?.minutes || 0)
  : Number(member?.questions || 0).toLocaleString('pt-BR');
const getMemberStreak = (member) => Math.max(0, Number(member?.streak ?? member?.currentStreak ?? member?.studyStreak ?? 0) || 0);

const LevelBadge = ({ level = 1 }) => {
  const value = Math.max(1, Number(level) || 1);
  return <span className="inline-flex h-5 items-center gap-1 rounded-lg border border-zinc-200 bg-zinc-100 px-1.5 text-[7px] font-black uppercase tracking-wide text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300" title={`Nível ${value}`}><Award size={9}/><span className="opacity-70">Nível</span><strong className="text-[9px]">{value}</strong></span>;
};

const StreakBadge = ({ member }) => {
  const streak = getMemberStreak(member);
  return <span className={`inline-flex h-5 items-center gap-1 rounded-lg border px-1.5 text-[8px] font-black ${streak > 0 ? 'border-orange-200 bg-orange-50 text-orange-700 dark:border-orange-900/60 dark:bg-orange-950/30 dark:text-orange-300' : 'border-zinc-200 bg-zinc-50 text-zinc-400 dark:border-zinc-700 dark:bg-zinc-800/70'}`} title={`${streak} ${streak === 1 ? 'dia' : 'dias'} de sequência`}><Flame size={10} fill={streak > 0 ? 'currentColor' : 'none'}/>{streak}d</span>;
};
...
const LiveIndicator = ({ active, reduceMotion = false }) => active ? (
  <span className="relative inline-flex h-2.5 w-2.5 shrink-0" title="Estudando agora" aria-label="Estudando agora">
    {!reduceMotion && <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-70" />}
    <span className="relative inline-flex h-2.5 w-2.5 rounded-full border border-white bg-emerald-500 dark:border-zinc-900" />
  </span>
) : null;

const TopCard = ({ member, position, metric, reduceMotion, onOpen, nowMs }) => {
  ...
  return <Motion.button type="button" disabled={!member} onClick={() => member && onOpen(member, position)} initial={reduceMotion ? false : { opacity: 0, y: 18, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: reduceMotion ? 0 : position * 0.08, type: 'spring', stiffness: 170, damping: 18 }} whileHover={reduceMotion || !member ? undefined : { y: -4 }} className="flex min-w-0 flex-col items-center justify-end text-left disabled:cursor-default">
    ...
    <div className="mt-3 flex w-full items-center justify-center gap-1.5"><p className="truncate text-center text-[9px] font-black text-zinc-900 dark:text-white sm:text-xs">{member?.displayName || 'Posição aberta'}</p><LiveIndicator active={member && isLiveRankingMember(member, nowMs)} reduceMotion={reduceMotion}/></div>
    <p className="mt-0.5 max-w-full truncate text-center text-[8px] font-black text-zinc-600 dark:text-zinc-300 sm:text-[10px]">{member ? <>{metricValue(member, metric)}{metric === 'questions' ? ' questões' : ''}</> : '—'}</p>
...
const RankingRowsSkeleton = () => <div className="space-y-2 bg-zinc-50/70 p-3 dark:bg-zinc-950/30" aria-label="Carregando classificação" aria-busy="true">
  {[0, 1, 2, 3, 4].map((item) => <div key={item} className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900"><span className="h-8 w-8 animate-pulse rounded-lg bg-zinc-200 dark:bg-zinc-800"/><span className="h-10 w-10 animate-pulse rounded-full bg-zinc-200 dark:bg-zinc-800"/><span className="h-3 flex-1 animate-pulse rounded bg-zinc-200 dark:bg-zinc-800"/><span className="hidden h-3 w-20 animate-pulse rounded bg-zinc-200 dark:bg-zinc-800 sm:block"/></div>)}
</div>;
...
const RankingPage = ({ user, levelData, studyStreak = null }) => {
  ...
  const sorted = useMemo(() => sortGeneralRankingMembers(members, metric).map((member) => (
    (member.uid || member.id) === user?.uid && studyStreak !== null
      ? { ...member, streak: Math.max(0, Number(studyStreak || 0)) }
      : member
  )), [members, metric, studyStreak, user?.uid]);
  ...
  const own = ownIndex >= 0 ? sorted[ownIndex] : { displayName: user?.displayName || 'Você', level: levelData?.currentLevel || 1, streak: Math.max(0, Number(studyStreak ?? levelData?.profile?.streak ?? 0)), leagueId: levelData?.leagueId || 'iron', minutes: 0, questions: 0, accuracy: 0 };
  ...
  return <div className="mx-auto min-h-full w-full min-w-0 max-w-7xl px-3 pb-28 pt-3 sm:px-5 lg:px-7">
    ...
    <header className="mb-4">
      ...
      <p className="mt-1 text-xs font-semibold text-zinc-500 dark:text-zinc-400">{metric === 'minutes' ? 'Ordenado pelo tempo de estudo' : 'Ordenado pelo total de questões'} · atividade acadêmica nos últimos 30 dias</p>
    </header>
    <section className="min-w-0 overflow-hidden rounded-[1.5rem] border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900 sm:rounded-[1.75rem]">
      <div className="relative bg-gradient-to-b from-zinc-50 to-zinc-100 px-2 pt-4 dark:from-zinc-900 dark:to-zinc-950 sm:px-7 sm:pt-6">
        ...
        <div className="relative mx-auto grid max-w-3xl grid-cols-3 items-end gap-1.5 sm:gap-4">{[1, 0, 2].map((index) => <TopCard key={index} member={sorted[index]} position={index + 1} metric={metric} reduceMotion={reduceMotion} onOpen={openPublicProfile} nowMs={nowMs}/>)}</div>
      </div>
      <div className="border-t border-zinc-200 dark:border-zinc-800">
        ...
        {loading ? <RankingRowsSkeleton/> : !sorted.length ? <div className="p-10 text-center"><BarChart3 className="mx-auto mb-2 text-zinc-300"/><p className="text-xs font-black text-zinc-600 dark:text-zinc-300">Nenhum usuário ativo no ranking</p></div> : <div className="space-y-2 bg-zinc-50/70 p-2 dark:bg-zinc-950/30 sm:p-3">{sorted.map((member, index) => {
          ...
          return <button type="button" onClick={() => openPublicProfile(member, index + 1)} key={member.uid || member.id} style={{ contentVisibility: 'auto', containIntrinsicSize: '64px' }} className={`grid w-full grid-cols-[32px_1fr] items-center gap-2 rounded-xl border px-2.5 py-2.5 text-left transition hover:-translate-y-0.5 hover:shadow-md sm:grid-cols-[40px_minmax(160px,1fr)_90px_78px_70px] sm:gap-3 sm:px-4 ${index < 3 ? topRowTone[index] : 'border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900'} ${ownRow ? 'ring-2 ring-red-500/25' : ''}`}>
            ...
            <div className="flex min-w-0 items-center gap-2"><Avatar member={member} className="h-10 w-10"/><div className="min-w-0"><div className="flex items-center gap-1.5"><p className="truncate text-[11px] font-black text-zinc-800 dark:text-white sm:text-xs">{member.displayName}{ownRow ? ' · você' : ''}</p><LiveIndicator active={isLiveRankingMember(member, nowMs)} reduceMotion={reduceMotion}/><Movement member={member} metric={metric}/></div><div className="mt-1 flex flex-wrap items-center gap-1"><LevelBadge level={member.level}/><StreakBadge member={member}/>{LEAGUES_ENABLED ? <span className="inline-flex h-5 items-center gap-0.5 rounded-lg px-1.5 text-[7px] font-black uppercase text-white" style={{ backgroundColor: league.color }}><Shield size={8}/>{league.name}</span> : null}</div></div></div>
